CLASS zcl_17_deletion_duplicate_ashm DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_17_deletion_duplicate_ashm IMPLEMENTATION.
...
out->write( 'Contenido antes de ELIMINAR DUPLICADOS ADYACENTES' ).
...
OUT->write( '  ' ).
out->write( 'Contenido después de ELIMINAR DUPLICADOS ADYACENTES' ).
out->write( 'No se eliminó nada; los valores clave no son adyacentes' ).
out->write( 'Ordenar la tabla antes de ELIMINAR DUPLICADOS ADYACENTES' ).
out->write( flights ).
out->write( ` ` ).
...
OUT->write( '  ' ).
out->write( 'Contenido después de ELIMINAR DUPLICADOS ADYACENTES' ).
out->write( 'No se eliminó nada: ABAP compara todos los valores clave, incluida la fecha_vuelo, que es diferente para cada entrada' ).
out->write( flights ).
out->write( ` ` ).
...
out->write( 'Contenido después de ELIMINAR DUPLICADOS ADYACENTES con COMPARACIÓN y lista de campos' ).
out->write( 'Se han eliminado las entradas con valores idénticos de Carrier_id y Connection_id' ).
out->write( flights ).
