CLASS zcl_calculations_times_ashm DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_calculations_times_ashm IMPLEMENTATION.
 ...
 out->write('**************************************************************************************').
  timestamp1 = utclong_current( ).
 out->write( |Hora UTC actual { timestamp1 }| ).

 timestamp2 = utclong_add( val = timestamp1 days = 7 ).
 out->write( |Se agregaron 7 días a la hora UTC actual { timestamp2 }| ).

 difference = utclong_diff( high = timestamp2 low = timestamp1 ).
 out->write( |Diferencia entre marcas de tiempo en segundos: { difference }| ).

 out->write( |Diferencia entre marcas de tiempo en días: { difference / 3600 / 24 }| ).
 ...
 out->write( |Marca de tiempo UTC dividida en fecha (tipo D) y hora (tipo T)| ).
 out->write( |según la zona horaria del usuario (cl_abap_context_info=>get_user_time_zone()).| ).
 out->write( |{ date_user date = user }, { time_user time = user }| ).
